...
* 프로그램에서 원하는 처리를 하려면 필요한 데이터 입력을 할 수 있어야 한다.
* 프로그램 실행 도중 정수, 실수, 문자, 문자열 입력하는 방법   

1. scanf 함수   

 * 키보드에서 타이핑 하는 모든 내용은 문자로 인식됨 (Enter포함)   
 * 데이터를 연산 가능한 정수나 실수로 사용하는 변환 과정이 필요 ⇒ scanf 함수   
...
* scanf 함수에서 변수명 지정시, &붙여야함   
* scanf 함수에서 사용한 변환 문자와 맞는 형태의 데이터를 입력해야 함 
※ short형 변수에 입력할 때 %d 사용하면 안되는 이유   
   : short(2byte) - %d(정수 4byte) = 더 큰 값이기 때문에 오류 가능성을 차단함   
...
* 문자열 : char배열 , %s 변환 문자 사용, &기호 붙이지 않음!   
∵ 배열 이름 자체가 주소이기 때문에 &기호 필요 없음!!   
